...
- **按需加载**：3D 部分在首屏只加载 1.5 KB 的轻量入口；Three.js、3D 运行时和两个机械组件在实验场进入视口后才请求。卡片按接近视口的顺序、利用浏览器空闲时段逐个初始化，不提前创建远离视口的 WebGL 上下文。
- **运行预算**：3D 场景仅在可见时渲染，桌面端上限 30 FPS，窄屏或省流量设备上限 20 FPS；窄屏降低像素比并关闭实时阴影。背景星场使用相同的 30/20 FPS 自适应上限。
- **媒体加载**：四段装饰视频使用海报占位，只有对应区域进入视口后才附加视频源并开始下载；减弱动态或省流量模式不会主动请求 MP4，已载入的视频会立即暂停。
